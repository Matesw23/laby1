...
public:
	...
	void Begin() {
		BeginDrawing();
		DrawTextureEx(background,BackgroundBase,0,scale,WHITE);
...
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			...
			Rectangle lr = { transform.position.x - 6.f, transform.position.y - LASER_LENGTH, 4.f, LASER_LENGTH };
	...
	float angleDeg)
{
	...
	Vector2 vel{ speed*sinf(angleRad), -speed*cosf(angleRad) };
...
public:
	...
	void Update(float dt) override {
		if (alive) {
			...
			if (IsKeyDown(KEY_Q)) transform.rotation -= rotspeed * dt;
			if (IsKeyDown(KEY_E)) transform.rotation += rotspeed * dt;
		}
		else {
	...
	void Draw() const override {
		...
		float textureW = (float)texture.width*scale;
		float textureH = (float)texture.height*scale;
		Rectangle source = { 0, 0, (float)texture.width, (float)texture.height };
		Rectangle dest = {  transform.position.x, transform.position.y, textureW, textureH };
		Vector2 origin = { textureW / 2, textureH / 2 };  

	
		float DrawRotation = transform.rotation;


		DrawTexturePro(texture, source, dest,origin, DrawRotation, WHITE);
	}
...
private:
	...
	static constexpr int C_HEIGHT = 1080;
